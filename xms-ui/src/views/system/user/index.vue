  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="20" :xs="24">
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <!-- <el-table-column label="谷歌绑定码" align="center" key="googleKey" prop="googleKey"  /> -->
          ...
          >
            <template slot-scope="scope">
              ...
              v-if="scope.row.userId === 1"
                size="mini"
                type="text"
                @click="handleGoogle(scope.row)"
              >绑定谷歌</el-button>
              <el-dropdown v-if="scope.row.userId !== 1" v-hasPermi="['system:user:resetPwd', 'system:user:edit']" size="mini" @command="(command) => handleCommand(command, scope.row)">
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body width="600px">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="登陆时长">
              <el-input-number v-model="form.loginDuration"  label="登陆时长(小时)" ></el-input-number>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
import { listUser, getUser, delUser, addUser, updateUser, resetUserPwd, changeUserStatus, deptTreeSelect,googleUser } from "@/api/system/user";
...
export default {
  ...
  methods: {
    ...
    handleGoogle(row){
      const userIds = row.userId || this.ids;
      this.$modal.confirm('是否确认申请'+row.userName+'的谷歌验证器私钥？').then(function() {
        return googleUser(userIds);
      }).then((response) => {
        console.log(response);
        this.getList();
        this.$modal.msgSuccess("申请成功，您的私钥是"+response.data+"请在谷歌验证器APP发起绑定");
      }).catch(() => {});
    },
    /** 导出按钮操作 */
    handleExport() {
