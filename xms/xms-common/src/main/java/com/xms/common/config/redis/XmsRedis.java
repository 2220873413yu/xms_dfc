...
public class XmsRedis {
	 ...
	 * @return
	 */
	public void set(final String key, Object value, Long expireTime, final TimeUnit timeUnit) {
	 ...
	 */
	public void setEx(Object key, Object value, Duration timeout) {
	 ...
	 * @return
	 */
	public <T> T cacheHGet(String key, String userId, Supplier<T> loader) {
		T value = this.hGet(key, userId);
	...
	@Nullable
	public <T> T get(String key, Supplier<T> loader, Long expireTime, final TimeUnit timeUnit) {
		T value = this.get(key);
		if (value != null) {
			return value;
		}
	...
	public void mSet(Object... keysValues) {
		redisTemplate.opsForValue().multiSet(CollectionUtil.toMap(keysValues));
	}
	 ...
	 * 当 key 存在但不是列表类型时，返回一个错误。
	 */
	public Long lPushAll(String key, Collection data) {
	 ...
	 * 当 key 存在但不是列表类型时，返回一个错误。
	 */
	public Long rPushAll(String key, List<?> data) {
	...
	/**
	 * @param bitIndex 位图的索引
	 * @param key      该bitmap的key
	 * @param flag     是否
	 */
	public void likePost(Integer bitIndex, String key, boolean flag) {
		// 将userId和postId转换为对应的二进制位
		// 使用bitset命令设置对应的位为1，表示用户已经占用
		redisTemplate.opsForValue().setBit(key, bitIndex, flag);
	}
	 ...
	 * @param key      该bitmap的key
	 */
	public boolean hasLikedPost(Integer bitIndex, String key) {
		// 使用bitset命令获取对应的位，如果为1，表示用户已经点赞
		return redisTemplate.opsForValue().getBit(key, bitIndex);
	}

	/**
	 * 统计是否截止多少位之前的被占用的连续位数
	 *
	 * @return
	 */
	public Integer signCount(Integer bitIndex, String key) {

		// 5.获取截止bitIndex为止的所有的占用记录，返回的是一个十进制的数字 BITFIELD
		List<Long> result = redisTemplate.opsForValue().bitField(
			key,
			BitFieldSubCommands.create()
				.get(BitFieldSubCommands.BitFieldType.unsigned(bitIndex)).valueAt(0)
		);
	 ...
	 * @return
	 */
	public Long getBitCount(String key, Long bitIndex) {
		// int bitCount = Long.SIZE - Long.numberOfLeadingZeros(num);
		// 获取 Redis 连接
		return redisTemplate.execute(connection -> connection.bitCount(key.getBytes(), 0, bitIndex - 1), true);
	}
	 ...
	 * @return
	 */
	public Long getZeroBitCount(String key, Long bitIndex) {
		// 获取 Redis 连接
		Long count = redisTemplate.execute(connection -> connection.bitCount(key.getBytes(), 0, bitIndex - 1), true);
