...
public class WorkerWrapper<T, V> {
	 ...
	 */
	private T param;
	private IWorkerPlus<T, V> worker;
	private ICallbackPlus<T, V> callback;
	/**
	...
	private WorkerWrapper(String id, IWorkerPlus<T, V> worker, T param, ICallbackPlus<T, V> callback) {
		if (worker == null) {
		...
		//允许不设置回调
		if (callback == null) {
			callback = new DefaultCallbackPlus<>();
		}
		this.callback = callback;
	}

	private static ExecutorService getExecutorService(ExecutorRegion executorRegion, Long id) {
		//Long.parseLong(wrapper.getId())
		ThreadExecutor userVirtualThreadExecutor = executorRegion.getBlockUserVirtualThreadExecutor(id, false);
		return (ExecutorService) userVirtualThreadExecutor.getRealExecutor();
	}
	 ...
	 */
	private void work(ExecutorRegion executorService, WorkerWrapper fromWrapper, long remainTime, Map<String, WorkerWrapper> forParamUseWrappers) {
	...
	public void work(ExecutorRegion executorService, long remainTime, Map<String, WorkerWrapper> forParamUseWrappers) {
	 ...
	 * 进行下一个任务
	 */
	private void beginNext(ExecutorRegion executorService, long now, long remainTime) {
		//花费的时间
		...
		for (int i = 0; i < nextWrappers.size(); i++) {
			...
			futures[i] = CompletableFuture.runAsync(() -> nextWrappers.get(finalI)
				...
				getExecutorService(executorService, Long.parseLong(nextWrappers.get(finalI).getId())));
		}
		try {
	...
	private synchronized void doDependsJobs(ExecutorRegion executorService, List<DependWrapper> dependWrappers, WorkerWrapper fromWrapper, long now, long remainTime) {
	...
	private WorkResult<V> workerDoJob() {
		...
		try {
			...
			//执行耗时操作
			V resultValue = worker.actionVirtual(param, forParamUseWrappers);
	...
	public static class Builder<W, C> {
		 ...
		 */
		private W param;
		private IWorkerPlus<W, C> worker;
		private ICallbackPlus<W, C> callback;
		/**
		 * 自己后面的所有
		 */
		private List<WorkerWrapper<?, ?>> nextWrappers;
		...
		public Builder<W, C> worker(IWorkerPlus<W, C> worker) {
			this.worker = worker;
			return this;
		}
		...
		public Builder<W, C> callback(ICallbackPlus<W, C> callback) {
			this.callback = callback;
			return this;
		}
